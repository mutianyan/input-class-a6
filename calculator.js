...
"#oneKey", "#twoKeys", "#threeKeys", "#fiveKeys"];
...
var num_of_keys = 5; // the default 

$("#oneKey").click(function(event) {
	num_of_keys = 1;
	console.log(num_of_keys);
	startLooping();
})


$("#twoKeys").click(function(event) {
	num_of_keys = 2;
	console.log(num_of_keys);
	stopLooping();
})

$("#threeKeys").click(function(event) {
	num_of_keys = 3;
	console.log(num_of_keys);
	stopLooping();
})

$("#fiveKeys").click(function(event) {
	num_of_keys = 5;
	console.log(num_of_keys);
	stopLooping();
...
function startLooping(){
		intervalID = window.setInterval(selectNext, 1500);
}
...
$(document).keypress(function(event) {
	if (num_of_keys == 5){
		if (event.key == "d") { // arrow left 
			selectNext();	
		} else if (event.key == "a") { // arrow right
			selectPrevious();
		} else if (event.key == "w") { // arrow up
			selectUp();
		} else if (event.key == "s") { // arrow down
			selectDown();
		} else if (event.key== "e") { // space
			clickSelectedItem();
		}
	} else if(num_of_keys == 3){
		if (event.key == "d") {
			selectNext();	
		} else if (event.key == "a") {
			selectPrevious();
	...
	} else if(num_of_keys == 2){
		if (event.key == "d"){
			selectNext();
		} else if (event.key == "e"){
			clickSelectedItem();
		}
	} else if (num_of_keys == 1){
		if (event.key == "e"){
				clickSelectedItem();
				// clearInterval(intervalID);
		}
...
// two button interface using NEXT and SELECT
// three button interface using NEXT, PREVIOUS, and SELECT
// five button interfacce with four way navigation and SELECT

// one button interfacce -- using JS setInterval()
